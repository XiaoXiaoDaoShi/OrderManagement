package servlet;

import java.io.IOException;
import java.sql.Date;
import java.util.List;

import javax.servlet.ServletException;
import javax.servlet.annotation.WebServlet;
import javax.servlet.http.HttpServlet;
import javax.servlet.http.HttpServletRequest;
import javax.servlet.http.HttpServletResponse;

import com.alipay.api.AlipayApiException;
import com.alipay.api.AlipayClient;
import com.alipay.api.DefaultAlipayClient;
import com.alipay.api.request.AlipayTradePagePayRequest;

import pay.AlipayConfig;
import pojo.Dish;
import pojo.OrderForm;
import service.OrderFormService;

/**
 * Servlet implementation class CreateOrderSevlet
 */
//创建订单
@WebServlet("/CreateOrderSevlet")
public class CreateOrderSevlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public CreateOrderSevlet() {
        super();
        // TODO Auto-generated constructor stub
    }

	/**
	 * @see HttpServlet#doGet(HttpServletRequest request, HttpServletResponse response)
	 */
    //创建订单
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		//获得基本订单基本信息
		String orderGuestName = request.getParameter("orderGuestName");
		String orderGuestPhone = request.getParameter("orderGuestPhone");
		String orderGetWay = request.getParameter("orderGetWay");
		String orderRemark = request.getParameter("orderRemark");
		String orderPrice = request.getParameter("totalPrice");
		Date orderDate = new Date(new java.util.Date().getTime());
		
		System.out.println(orderPrice);
		
		//创建订单
		OrderForm orderForm = new OrderForm(Integer.valueOf(orderPrice), orderGuestName, orderGuestPhone, Integer.valueOf(orderGetWay), orderDate, orderRemark);
		
		OrderFormService os = new OrderFormService();
		int count = os.addOrderForm(orderForm);
		String msg = "";
		if(count>0) {
			msg = "1";
			List<Dish> orderDishList = (List<Dish>) request.getSession().getAttribute("orderDishList");
			System.out.println(orderDishList);
			for(int i=0;i<orderDishList.size();i++) {
				orderDishList.get(i).addCount();
			}
		}else {
			msg = "0";
		}
		this.pay(request, response, orderPrice);
//		response.getWriter().println(msg);
		
	}

	/**
	 * @see HttpServlet#doPost(HttpServletRequest request, HttpServletResponse response)
	 */
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		// TODO Auto-generated method stub
		doGet(request, response);
	}
	//支付功能
	private void pay(HttpServletRequest request, HttpServletResponse response, String price) throws ServletException, IOException{
		String APP_ID = "2016101500694894";
		String APP_PRIVATE_KEY = "";
		String FORMAT = "json";
		String CHARSET = "utf-8";
		String ALIPAY_PUBLIC_KEY = "";
		String SIGN_TYPE = "RSA2";
		AlipayClient alipayClient = new DefaultAlipayClient("https://openapi.alipaydev.com/gateway.do", APP_ID, APP_PRIVATE_KEY, FORMAT, CHARSET, ALIPAY_PUBLIC_KEY, SIGN_TYPE); //获得初始化的AlipayClient
		
	    AlipayTradePagePayRequest alipayRequest = new AlipayTradePagePayRequest();//创建API对应的request
	    alipayRequest.setReturnUrl(AlipayConfig.return_url);
	    alipayRequest.setNotifyUrl(AlipayConfig.notify_url);//在公共参数中设置回跳和通知地址
	    
	    
	    String trade_no = String.valueOf(new java.util.Date().getTime());
	    
	    String tradeInfo = "{" +
		        "\"out_trade_no\":\""+ trade_no+ "\"," +
		        "\"product_code\":\"FAST_INSTANT_TRADE_PAY\"," +
		        "\"total_amount\":" + price + ","+
		        "\"subject\":\"订餐\"}";
		        
	    alipayRequest.setBizContent(tradeInfo);
//	       
	  
	    String form="";
	    try {
	        form = alipayClient.pageExecute(alipayRequest).getBody(); //调用SDK生成表单
	    } catch (AlipayApiException e) {
	        e.printStackTrace();
	    }
	   response.setContentType("text/html;charset=" + CHARSET);
	    response.getWriter().write(form);//直接将完整的表单html输出到页面
	    response.getWriter().flush();
	   response.getWriter().close();
	}
}
